...
For the Vercel frontend, set `NEXT_PUBLIC_API_URL` to the lorMan Railway service URL ending in `/api` (for example, `https://lorman-backend-production.up.railway.app/api`) and redeploy. The frontend also accepts the host-only form and adds `/api` automatically. Do not use a URL belonging to another project. The frontend only uses localhost as an API fallback during local development.
